...
const EventBarCollection = ({events, size=5, handlers}) => {
  ...
  for (var i = 0; i < limit; i++) {
    if (!events[i])
      continue;
    rendered_events.push(<EventBar key={events[i].id} users={events[i].users} capacity={events[i].capacity} host = {events[i].host} handlers={handlers} games = {events[i].games} details = {events[i].details} categories={events[i].categories}location={events[i].short_location} name={events[i].title} date={events[i].date} id={events[i].id}/>)
  }
